...
Undergraduate courses on statistics concentrate on theory, 
and use some spreadsheet as an universal computing tool.

For students statistics is **a lot of math formulas + Excel = difficult and boring**

Students works with artificial (clean)
and small data sets thus are unaware of problems related to
applying theory to practice and/or about the data definition/collection stage.

Change of concept is **urgently needed**. Student should be aware about
the **true** workflow of statistical analysis:

## How statistics should be taught (IMO at least)

**Start** from *true begining* and show students real *'value chain' of statistical analysis
not simpliefied one, with all its problems and dark corners: 

* Poor definitions (imprecise, complex, unintuitive, contradictory);

* complexity of statistical data collection and imprecise measurement; 

* unreliable data (incomplete, unavailable, erroneous, imputed, guessed).

**Finish** with *true end* and introduce students seriously not marginally with
quality publishing and (basics of) information visualization.  This implies 
introduction of modern software (R namely) and elementary programming,
ie. abandoning 4-horses-of apocalipse (pardon office suite)
or paradign of *point-and-click* as the only mode of work

* Office software has limits. Spreadsheets are good for number crunching,
but are not so good in: data cleaning, advanced graphics,
spatial analysis, team work, etc...
...
Mimo, że jeszcze gdzieś indziej też jest piękny świat\
(Natalia w Bruklinie/Kazik Staszewski)
...
is travelling for leisure. People travelling for other 
purposes has different needs/aims than those travelling to rest,
...
Two sides of tourism: supply side/demand side

BTW: demand = how much a product/service is desired; 
supply = how much the market can offer

Tourism supply statistics (accommodation statistics):
Data on rented accommodation ie. capacity and occupancy
...
Data on participation in tourism of the residents of the reporting country.
How collected? Surveys?

Most of the time, data on domestic and outbound
trips (where "outbound tourism" means residents
of a country travelling in another country) is collected via sample
...
No doubt in every reliable survey the population
has to be precisely defined ie 
3 dimensions of every surveyed unit should be fixed:
definition (**what**), time (**when** measured), space (*where*).
If at least one demension
is missing the data is **defective**.

Proposal: dissemination of defective data should be subject to public prosecution (joke)
...
indicators is prone to (significant) errors.
 ...
 Data collection methods from most to last reliable:
...
The most important argument why I will present momentarily and it concerns the basic approach of doing statistical analysis. 
...
## Summary: New Tools (hipster mode)
